        ...
        <div class="row">
            ...
            <div class="col-lg-12  px-5">
                ...
                <div class="chart-calculate-form">
                    ...
                    @endif
                    @if(isset($user->member))
                        <form method="POST" action="{{ route('admin.memberships.update', $user->id) }}" enctype="multipart/form-data">
                            @method('PUT')
                    @else
                        <form method="POST" action="{{ route('admin.memberships.store', $user->id) }}" enctype="multipart/form-data">
                    @endif
                            @csrf
                            <div class="row">
                                <div class="col-sm-6">
                                ...
                                <div class="col-sm-6">
                                    <input type="file" placeholder="Profile Photo" name="profile_photo" class="@error('profile_photo') is-invalid @enderror pt-2" @if(!$user->member) ? required @endif>
                                    @error('profile_photo')
                                        <span class="invalid-feedback mb-3" role="alert">
                                            <strong>{{ $message }}</strong>
                                        </span>
